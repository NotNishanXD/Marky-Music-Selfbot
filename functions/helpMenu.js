...
async function generateHomepageImage(categories, errorMsg = null) {
  ...
  ctx.textAlign = "center";
  ctx.fillText("MARKY MUSIC SELFBOT — COMMANDS", WIDTH / 2, 60);

  ctx.fillStyle = PALETTE.dim;
  ctx.font = FONTS.subtitle;
  ctx.fillText("Premium command dashboard — Created by Marky", WIDTH / 2, 86);
  ...
  ctx.textAlign = "center";
  ctx.fillText("Created by Marky • Premium Music Selfbot", WIDTH / 2, HEIGHT - 16);
...
async function generateCategoryImage(commands, categoryName) {
  ...
  ctx.fillStyle = PALETTE.dim;
  ctx.font = FONTS.subtitle;
  ctx.fillText("Created by Marky", WIDTH / 2, 86);
...
function generateHomepageText(categories, errorMsg = null) {
  ...
  let out = "```\nMARKY MUSIC SELFBOT - CATEGORIES\n\n";
